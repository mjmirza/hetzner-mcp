...
async function main(): Promise<void> {
  ...
  assert("security: reject percent-encoded path traversal", percentTravOk);

  let backslashOk = true;
  try {
    normalizePath("/servers/\\foo");
    backslashOk = false;
  } catch {
    /* expected */
  }
  assert("security: reject backslash normalization bypass", backslashOk);

  let malformedPercentOk = true;
  try {
    normalizePath("/servers/%g1");
  ...
  let validPathOk = true;
  try {
    const res = normalizePath("/servers/%20foo");
    if (res !== "/servers/%20foo") validPathOk = false;
  } catch {
    validPathOk = false;
  }
  assert("security: allow valid percent-encoded paths", validPathOk);
