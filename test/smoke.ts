...
async function main(): Promise<void> {
  ...
  assert("security: reject path traversal (percent-encoded dots)", pctsTravOk);
  ...
  assert("security: reject path traversal (backslashes)", backslashOk);

  let pctBackslashOk = true;
  try {
    normalizePath("/servers%5c..%5c..%5cadmin");
    pctBackslashOk = false;
  } catch {
    /* expected */
  }
  assert("security: reject path traversal (percent-encoded backslashes)", pctBackslashOk);

  let invalidPctOk = true;
  try {
    normalizePath("/servers/%g1/admin");
    invalidPctOk = false;
  } catch {
    /* expected */
  }
  assert("security: reject invalid percent-encoding", invalidPctOk);
  ...
  const total = results.length + 6 + costChecks.length;
