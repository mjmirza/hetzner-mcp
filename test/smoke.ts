...
async function main(): Promise<void> {
  ...
  assert("security: reject backslash path traversal", bsOk);

  let encBsOk = true;
  try {
    normalizePath("/servers/%2e%2e%5c%2e%2e%5cadmin");
    encBsOk = false;
  } catch {
    /* expected */
  }
  assert("security: reject percent-encoded backslash path traversal", encBsOk);
