...
async function main(): Promise<void> {
  ...
  try {
    normalizePath("/servers/%2e%2e/pricing");
    encodedTravOk = false;
  } catch {
    /* expected */
  }
  ...
  let invalidEncodingOk = true;
  try {
    normalizePath("/servers/%ff/test");
    invalidEncodingOk = false;
  } catch {
    /* expected */
  }
  assert("security: reject invalid percent encoding", invalidEncodingOk);
