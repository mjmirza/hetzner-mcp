...
function assertRejects(label: string, fn: () => void): boolean {
  try {
    fn();
    return assert(label, false);
  } catch {
    return assert(label, true);
  }
}
...
async function main(): Promise<void> {
  ...
  const secChecks = [
    assertRejects("security: reject full URL path", () => normalizePath("https://evil.example.com/steal")),
    assertRejects("security: reject path traversal", () => normalizePath("/servers/../../admin")),
    assertRejects("security: reject encoded path traversal", () => normalizePath("/servers/%2e%2e/%2e%2e/admin")),
    assertRejects("security: reject backslash traversal", () => normalizePath("/servers\\..\\admin")),
    assertRejects("security: reject encoded backslash", () => normalizePath("/servers%5c..%5cadmin")),
    assertRejects("security: reject invalid percent encoding", () => normalizePath("/servers/%E0%A0")),
    assertRejects("security: reject encoded control chars", () => normalizePath("/servers/%0a/admin")),
  ];
